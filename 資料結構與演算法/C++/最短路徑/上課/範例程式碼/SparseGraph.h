...
private:
    ...
    vector<vector<Edge<Weight> *>> g;  //Adjacency list--->使用vector實作
public:
    //傳入節點數量，有向圖或無向圖
    SparseGraph(int n, bool directed) {
        ...
        for (int i = 0; i < n; i++) {
            //每個節點 都預設是NULL(沒有與其他節點相連)
            g.push_back(vector<Edge<Weight> *>());
        }
    }
    ~SparseGraph() {
        //釋放掉Adjacency List中所有的Edge類指標
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (g[i][j] != NULL) {
                    delete g[i][j];
    ...
    bool hasEdge(int v, int w) {
        ...
        for (int i = 0; i < g[v].size(); i++) {  //遍歷節點v的vector
            if (g[v][i]->other(v) == w) {        //使用Edge類中的other去判斷 回傳的節點是否為w
                return true;
            }
        }
        //查無結果--->沒有與w相連
    ...
    public:
        ...
        //迭代器的起點(節點v中vector的第一個元素 也就是第一個紀錄的邊)
        Edge<Weight> *begin() {
            if (G.g[v].size()) {   //檢查圖的節點v是否有元素--->有元素才返回
                index = 0;         //更新 當前索引位置
                return G.g[v][0];  //回傳該元素(也就是邊的物件指標)
            }
            //沒有元素可以遍歷--->回傳NULL(空指標)
            return NULL;
        }
        ...
        Edge<Weight> *next() {
            ...
            if (index < G.g[v].size()) {  //沒有越界--->代表下一個位置 是有元素的
                return G.g[v][index];     //回傳該元素(也就是邊的物件指標)
            }
            return NULL;  //沒有下一個元素--->回傳NULL(空指標)
        }
